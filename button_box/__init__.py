...
def _passwordTkinter(
    text="", title="", default="", mask="*", root=None, timeout=None
):
    """Displays a message box with text input, and OK & Cancel buttons. Typed characters appear as *. Returns the text entered, or None if Cancel was clicked."""
    assert TKINTER_IMPORT_SUCCEEDED, "Tkinter is required for pymsgbox"
    text = str(text)
    return __fillablebox(
        text, title, default, mask=mask, root=root, timeout=timeout
    )
...
def _buttonbox(msg, title, choices, root=None, timeout=None):
    ...
    messageWidget.pack(
        side=tk.TOP, expand=tk.YES, fill=tk.X, padx="3m", pady="3m"
    )
...
def __fillablebox(
    msg, title="", default="", mask=None, root=None, timeout=None
):
    """
